...
{
  mutt_debug(1, "entering help_msg_open: %d, %s\n", msgno, m->hdrs[msgno]->env->subject);

  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/%s", m->realpath, m->hdrs[msgno]->path);

  m->hdrs[msgno]->read = true; /* reset a probably previously set unread status */
